...
scripts/kfind.sh 우리팀                # 방·친구를 한 번에 조회 (보내기 전에 확인)
scripts/kfind.sh --rooms 우리팀        # 방만
...
[카톡/work] 철수쌤: 확인해볼게요
[카톡/work] 김감독: 이게 낫다  ↩︎(cut-03.mp4)
```

마지막 줄처럼 `↩︎(…)` 가 붙으면 **답장(인용)** 이고 괄호 안이 그 대상이다.
카톡 답장은 본문만 보면 무엇을 가리키는지 알 수 없다 — 파일 셋을 보내놓고 "이게 낫다" 만
오면 사람에게 되물어야 한다. 그래서 폴링이 대상을 같이 찍는다.
...
고르기 전에 `kfind.sh <조각>` 으로 몇 군데에 걸리는지 보고, 전송할 때는 래퍼가 한 번 더 센다
(아래 「함정」의 방 검증).
...
### 방 검색은 부분일치다 — 보내기 전에 세어본다
`kmsg` 도 `kakaocli` 도 방을 **이름 부분일치**로 찾는다. 정확일치 옵션이 없다.
그래서 `search` 조각이 두 방에 걸리면 **엉뚱한 방으로 나가고, 나간 건 되돌릴 수 없다.**
실제로 걸릴 뻔했다 — 개인톡에 보내려던 조각이 같은 낱말을 품은 오픈채팅(2000명대)과
다른 그룹방에도 걸렸고, 실측하니 개인톡이 아니라 그룹방이 잡혔다.

막는 쪽은 둘로 나뉜다. **고르는 건 사람이 하고, 멈추는 건 도구가 한다.**

```bash
scripts/kfind.sh 우리팀       # 고르기 — 걸리는 방을 전부 펼쳐 보여준다
```

```
── 방 ──────────────────────────────────────────
  222222222222222222   오픈채팅   2593명  우리팀 정보 공유방
  111111111111111      그룹          8명  소중한 우리팀 사람들
  ⚠ 2 곳에 걸린다 — 이 조각으로는 전송하면 안 된다.
    각 방의 전체 이름 중 **다른 방에 없는** 조각을 골라 rooms.json 의 search 에 넣어라.
```

`ksend.sh` · `kimg.sh` · `kvid.sh` 는 보내기 직전에 같은 것을 DB 로 확인하고,
**하나가 아니면 전송을 멈춘다**(`klib.sh` 의 `kroom_verify`).

```
⚠ '우리팀' 이 방 2개에 걸린다 — 엉뚱한 방으로 갈 수 있어 멈춘다:
    222222222222222222
    111111111111111
전송을 멈췄다 — rooms.json 의 search 를 더 좁게 고쳐라 (방: work)
```

푸는 방법은 하나다. **`rooms.json` 의 `search` 를 더 좁은 조각으로 고친다.**
`KROOM_SKIP_VERIFY=1` 로 끌 수는 있지만, 끄면 `room verify.skipped` 가 로그에 남는다 —
껐다는 사실 자체가 나중에 보여야 한다.

방 이름은 **세 군데에 나뉘어 있다.** 한 군데만 보면 오탐이 난다.

| 테이블 | 무엇 |
|---|---|
| `NTChatRoom.chatName` | 대부분 **빈 문자열**이다 |
| `NTOpenLink.linkName` | 오픈채팅 이름 |
| `NTChatMeta` (`type=3`) | 일반 그룹방 이름 — 여기에만 있는 방이 흔하다 |

**1:1 방은 이름이 아예 없다.** 이름으로는 원리적으로 못 고른다.
`kfind.sh --friends` 가 대화명·`userId`·`directChatId` 를 보여주니 그쪽에서 짚어야 한다.

### 답장은 본문만 보면 무엇에 대한 답인지 모른다
카톡 답장(인용)의 대상은 본문에 없다. `NTChatMessage.attachment` JSON 안의
`src_logId` / `src_userId` / `src_message` 에 있다. **`referer` 컬럼은 0 이라 쓸모없다.**
`kpoll.py` 가 이걸 읽어 `↩︎(대상)` 으로 붙인다 — 없으면 파일 여러 개를 보내놓고
"이게 낫다" 를 받았을 때 매번 되물어야 한다.
