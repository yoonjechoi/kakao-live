...
**이름을 지어준 적 없는 그룹방은 그 세 군데 어디에도 없다.**
카톡이 화면에 보여주는 `홍길동, 김철수` 는 멤버 이름을 그 자리에서 이어 붙인 것이지
저장된 값이 아니다. **인원수와 무관하다** — 같은 3명 방이라도 이름을 지었으면 `NTChatMeta` 에
있고 아니면 없다. 이런 방은 `rooms.json` 의 `search` 에 **화면에 보이는 그대로**
`"최대건, 최인창"` 처럼 쉼표째 적으면 된다. `kroom_verify` 가 쉼표로 쪼개
**그 이름들이 전부 발화한 방**을 교집합으로 찾는다(로그에 `verify.by_members`).

> 여기서 `KROOM_SKIP_VERIFY=1` 이 떠오르면 참아라. 그건 오발송 방지를 끄는 것이다.
> **막히면 검증이 보는 범위를 넓혀야지 검증을 끄면 안 된다.**
...
### `kdel.sh` — 내가 보낸 메시지 지우기

```bash
./kdel.sh -r work --dry     # 창을 찾아 좌표만 계산
```

**창을 먼저 좌클릭해 활성화해야 우클릭 메뉴가 뜬다.** 이것이 없으면 합성 클릭이든
Quartz `CGEvent` 든 우클릭이 그냥 무시된다 — 이걸 모르고 반나절을 좌표·타이밍만 의심했다.

| | |
|---|---|
| 컨텍스트 메뉴 | **AX 트리에 안 잡힌다**(`menus of app: 0`). 좌표로만 누른다 |
| 메뉴 구성 | 오픈채팅엔 「모두에게서 삭제」·「가리기」, **일반 그룹엔 「나에게서만 삭제」만** |
| 삭제 절차 | 한 번이 아니라 **선택모드 → 체크박스 → 확인 → 삭제** 4단계 |
| 성공 판정 | **DB(NTChatMessage)에는 그대로 남는다.** 화면으로 확인해야 한다 |
| 시간 제한 | 「모두에게서 삭제」는 전송 후 **5분** 안에만 된다 |

말풍선 위치는 아직 사람이 화면을 보고 정해야 한다. 자동 검출은 없다.
